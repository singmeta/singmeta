 ...
 */
 ...
 router.post('/signUp', function (req, res, next) {
    ...
    db.collection('memberCount').findOne({name : 'totalCount'}, function(err, res){
        console.log(res.totalCount);
        var totalCount = res.totalCount;

        db.collection('member').insertOne({_id:totalCount + 1, email:req.body.email, password:req.body.password, nickname:req.body.nickname, phone:req.body.phone}, function(err, res){
            console.log(`${req.body.email} 저장완료`);

            db.collection('memberCount').updateOne({name : 'totalCount'}, { $inc : {totalCount:1} }, function(err, res){
                if(err) {
...
passport.use(new LocalStrategy({
    ...
    passReqToCallback : false // 아이디, 비번 이외에 다른 정보 검증 시
        ...
        db.collection('member').findOne({email : email_input}, function(err, res){
