...
class Room extends events_1.EventEmitter {
    ...
    send(client, message) {
        ...
        else if (message instanceof schema_1.Schema) {
            Protocol_1.send[Protocol_1.Protocol.ROOM_DATA_SCHEMA](client, message.constructor._typeid, message.encodeAll());
        }
        else {
        ...
        if (message instanceof schema_1.Schema) {
            ...
            while (numClients--) {
                ...
                if (options.except !== client) {
                    Protocol_1.send[Protocol_1.Protocol.ROOM_DATA_SCHEMA](client, typeId, encodedMessage);
                }
            }
        }
        else {
            // encode message with msgpack
            const encodedMessage = (!(message instanceof Buffer)) ? notepack_io_1.default.encode(message) : message;
            let numClients = this.clients.length;
            while (numClients--) {
                const client = this.clients[numClients];
                if (options.except !== client) {
